...
class ProfileCheckService {
  ...
  Future<String?> checkProfileRequired(
    ...
    if (usuario.isFuncionario) {
      print('🔍 Verificando perfil de funcionario para usuario ${usuario.idUsuario}...');
      try {
        final perfil = await _orgRepo.getPerfilFuncionarioByUsuario(usuario.idUsuario);
        if (perfil != null) {
    ...
    if (usuario.isVoluntario) {
      print('🔍 Verificando perfil de voluntario para usuario ${usuario.idUsuario}...');
      try {
        final perfil = await _volRepo.getPerfilByUsuario(usuario.idUsuario);
        if (perfil != null) {
          print('✅ Usuario voluntario tiene perfil: ${perfil.idPerfilVoluntario}');
          return null; // Tiene perfil, puede ir al home
        } else {
          print('⚠️ Usuario voluntario NO tiene perfil - necesita crear perfil');
